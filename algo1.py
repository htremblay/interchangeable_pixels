...
def moveRowOnTower(evol, k, start, end, tow, step=1) :
    """put the edges of the row on the tower"""
    for i in range(start, end+1, step) :
        evol.addAction(k, i, Direction.Up)
        combineTowers(evol, k, i, tow)
            ...
            for ind in contactsR[1:-1] : #remove the ends
                ...
                if si[1]-si[0] >= 3 :
                    ...
                    if si[0]-sinf[1] == 1 :
                        ...
                        else : #if a hole is created
                            evol.popAction()
                            evol.addAction(k+3, si[0]-1, Direction.Up)
                            moveRowOnTower(evol, k+1, si[0]-1, si[1]-2, towerPos)
                        ...
                        if connected : #if no hole is created
                            ...
                            evol.popAction()
                            evol.addAction(k+3, si[1], Direction.Up)
                            moveRowOnTower(evol, k+1, si[0]+1, si[1], towerPos)
                    
                    else : #part f
                        evol.addAction(k+1, si[0], Direction.DownLeft)
                        connected, notConnectedPixels = evol.currentImage.isConnected(whiteMode=True)
                        if not connected : #if a hole is created
                            if (k+2,si[0]-2) in notConnectedPixels :
                                evol.popAction()
                                evol.addAction(k+1, si[0]-1, Direction.Down)
                                moveRowOnTower(evol, k+1, si[0], si[0], towerPos)
                            else :
                                evol.popAction()
                                evol.addAction(k+1, si[0]+1, Direction.DownRight)
                                connected, notConnectedPixels = evol.currentImage.isConnected(whiteMode=True)
                                if not connected : #if a hole is created
                                    evol.popAction()
                                    evol.addAction(k+1, si[0]+2, Direction.Down)
                                    evol.addAction(k+1, si[0]+1, Direction.UpLeft)
            ...
            for a in range(len(ris)-1) :
                for j in range(ris[a], ris[a+1]) :
                    if evol.currentImage.getPixel(k+2, j+1)==0 :
                        ...
                        if not connected :
                            evol.popAction()
                            if evol.currentImage.getPixel(k+3, j+2)==0 :
                                evol.addAction(k+3, j+1, Direction.Up)
                                evol.addAction(k+1, j, Direction.Up)
                                combineTowers(evol, k+1, j, r[1]-1)
                            elif evol.currentImage.getPixel(k+4, j+2)==1 :
                                evol.addAction(k+3, j+1, Direction.Up)
                                evol.addAction(k+1, j, Direction.Up)
                                combineTowers(evol, k+1, j, r[1]-1)
                            else :
                                if evol.currentImage.getPixel(k+4, j+1)==0 :
                                    evol.addAction(k+3, j+2, Direction.UpLeft)
                                    evol.addAction(k+1, j, Direction.Up)
                                    combineTowers(evol, k+1, j, r[1]-1)
                                else :
                                    evol.addAction(k+3, j+1, Direction.UpRight)
                                    evol.addAction(k+1, j, Direction.DownRight)
