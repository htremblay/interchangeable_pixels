...
class BinaryImageDisplayer:
    ...
    def show(self, image: BinaryImage, subtitle: None | str = None, custom_pixels_lists=None, save=False, name_save=None) -> None:
        ...
        if save:
            if name_save is None:
                name_save = 'default_image'
            fig.savefig('./' + name_save + '.svg', dpi=500)
    ...
    def _show_legend(self, custom_pixels_lists=None) -> None:
        ...
        if self.show_isolated:
            isolated_white_pixels_label = 'Pixels blancs isolés'
            isolated_black_pixels_label = 'Pixels noirs isolés'
